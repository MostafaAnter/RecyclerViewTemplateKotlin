...
import com.recyclerviewtemplate.adapter.ToggleListAdapter
import java.util.*

class ToggleListActivity : AppCompatActivity() {
    private var recyclerView: RecyclerView? = null
    private var toolbar: Toolbar? = null
    private var swipeRefreshRecyclerList: SwipeRefreshLayout? = null
    private var fab: FloatingActionButton? = null
    private var mAdapter: ToggleListAdapter? = null
    ...
    private fun setAdapter() {
        ...
        mAdapter = ToggleListAdapter(this@ToggleListActivity, modelList)
        ...
        dividerItemDecoration.setDrawable(ContextCompat.getDrawable(this@ToggleListActivity, R.drawable.divider_recyclerview)!!)
        recyclerView!!.addItemDecoration(dividerItemDecoration)
        recyclerView!!.adapter = mAdapter
        mAdapter!!.SetOnItemClickListener(object : ToggleListAdapter.OnItemClickListener {
            override fun onItemClick(view: View?, position: Int, model: AbstractModel?) {

                //handle item click events here
                Toast.makeText(this@ToggleListActivity, "Hey " + model!!.title, Toast.LENGTH_SHORT).show()
            }
        })
        mAdapter!!.SetOnCheckedListener(object : ToggleListAdapter.OnCheckedListener {
            override fun onChecked(view: View?, isChecked: Boolean, position: Int, model: AbstractModel?) {
                Toast.makeText(this@ToggleListActivity, (if (isChecked) "Checked " else "Unchecked ") + model!!.title, Toast.LENGTH_SHORT).show()
            }
        })
    }
}