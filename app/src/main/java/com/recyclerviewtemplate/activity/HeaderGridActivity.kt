...
import com.recyclerviewtemplate.adapter.HeaderGridAdapter
import java.util.*

class HeaderGridActivity : AppCompatActivity() {
    private var recyclerView: RecyclerView? = null
    private var toolbar: Toolbar? = null
    private var swipeRefreshRecyclerList: SwipeRefreshLayout? = null
    private var fab: FloatingActionButton? = null
    private var mAdapter: HeaderGridAdapter? = null
    ...
    private fun setAdapter() {
        ...
        mAdapter = HeaderGridAdapter(this@HeaderGridActivity, modelList, "Header")
        recyclerView!!.setHasFixedSize(true)
        val layoutManager = GridLayoutManager(this@HeaderGridActivity, 2)
        recyclerView!!.addItemDecoration(GridMarginDecoration(this@HeaderGridActivity, 2, 2, 2, 2))
        recyclerView!!.layoutManager = layoutManager
        recyclerView!!.adapter = mAdapter
        mAdapter!!.SetOnItemClickListener(object : HeaderGridAdapter.OnItemClickListener {
            override fun onItemClick(view: View?, position: Int, model: AbstractModel?) {

                //handle item click events here
                Toast.makeText(this@HeaderGridActivity, "Hey " + model!!.title, Toast.LENGTH_SHORT).show()
            }
        })
        mAdapter!!.SetOnHeaderClickListener(object : HeaderGridAdapter.OnHeaderClickListener {
            override fun onHeaderClick(view: View?, headerTitle: String?) {

                //handle item click events here
                Toast.makeText(this@HeaderGridActivity, "Hey I am a header", Toast.LENGTH_SHORT).show()
            }
        })
    }
}