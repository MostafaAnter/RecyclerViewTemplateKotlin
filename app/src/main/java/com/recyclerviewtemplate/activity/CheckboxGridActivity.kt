...
import com.recyclerviewtemplate.adapter.CheckboxGridAdapter
import java.util.*

class CheckboxGridActivity : AppCompatActivity() {
    private var recyclerView: RecyclerView? = null
    private var toolbar: Toolbar? = null
    private var swipeRefreshRecyclerList: SwipeRefreshLayout? = null
    private var fab: FloatingActionButton? = null
    private var mAdapter: CheckboxGridAdapter? = null
    ...
    private fun setAdapter() {
        ...
        mAdapter = CheckboxGridAdapter(this@CheckboxGridActivity, modelList)
        recyclerView!!.setHasFixedSize(true)
        val layoutManager = GridLayoutManager(this@CheckboxGridActivity, 2)
        recyclerView!!.layoutManager = layoutManager
        recyclerView!!.adapter = mAdapter
        mAdapter!!.SetOnItemClickListener(object : CheckboxGridAdapter.OnItemClickListener {
            override fun onItemClick(view: View?, position: Int, model: AbstractModel?) {

                //handle item click events here
                Toast.makeText(this@CheckboxGridActivity, "Hey " + model!!.title, Toast.LENGTH_SHORT).show()
            }
        })
        mAdapter!!.SetOnCheckedListener(object : CheckboxGridAdapter.OnCheckedListener {
            override fun onChecked(view: View?, isChecked: Boolean, position: Int, model: AbstractModel?) {
                Toast.makeText(this@CheckboxGridActivity, (if (isChecked) "Checked " else "Unchecked ") + model!!.title, Toast.LENGTH_SHORT).show()
            }
        })
    }
}