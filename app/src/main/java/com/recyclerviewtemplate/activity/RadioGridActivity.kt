...
import com.recyclerviewtemplate.adapter.RadioGridAdapter
import java.util.*

class RadioGridActivity : AppCompatActivity() {
    private var recyclerView: RecyclerView? = null
    private var toolbar: Toolbar? = null
    private var swipeRefreshRecyclerList: SwipeRefreshLayout? = null
    private var fab: FloatingActionButton? = null
    private var mAdapter: RadioGridAdapter? = null
    private val modelList = ArrayList<AbstractModel>()
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_radio_grid)
        findViews()
    ...
    private fun setAdapter() {
        ...
        mAdapter = RadioGridAdapter(this@RadioGridActivity, modelList)
        recyclerView!!.setHasFixedSize(true)
        val layoutManager = GridLayoutManager(this@RadioGridActivity, 2)
        recyclerView!!.layoutManager = layoutManager
        recyclerView!!.adapter = mAdapter
        mAdapter!!.SetOnItemClickListener(object : RadioGridAdapter.OnItemClickListener {
            override fun onItemClick(view: View?, position: Int, model: AbstractModel?) {

                //handle item click events here
                Toast.makeText(this@RadioGridActivity, "Hey " + model!!.title, Toast.LENGTH_SHORT).show()
            }
        })
    }
}