...
import com.recyclerviewtemplate.adapter.SimpleGridCardsAdapter
import java.util.*

class SimpleGridCardsActivity : AppCompatActivity() {
    private var recyclerView: RecyclerView? = null
    private var toolbar: Toolbar? = null
    private var swipeRefreshRecyclerList: SwipeRefreshLayout? = null
    private var fab: FloatingActionButton? = null
    private var mAdapter: SimpleGridCardsAdapter? = null
    private val modelList = ArrayList<AbstractModel>()
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_simple_grid_cards)
        findViews()
    ...
    private fun setAdapter() {
        ...
        mAdapter = SimpleGridCardsAdapter(this@SimpleGridCardsActivity, modelList)
        recyclerView!!.setHasFixedSize(true)
        val layoutManager = GridLayoutManager(this@SimpleGridCardsActivity, 2)
        recyclerView!!.layoutManager = layoutManager
        recyclerView!!.adapter = mAdapter
        mAdapter!!.SetOnItemClickListener(object : SimpleGridCardsAdapter.OnItemClickListener {
            override fun onItemClick(view: View?, position: Int, model: AbstractModel?) {

                //handle item click events here
                Toast.makeText(this@SimpleGridCardsActivity, "Hey " + model!!.title, Toast.LENGTH_SHORT).show()
            }
        })
    }
}