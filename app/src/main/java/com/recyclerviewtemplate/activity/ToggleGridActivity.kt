...
import com.recyclerviewtemplate.adapter.ToggleGridAdapter
import java.util.*

class ToggleGridActivity : AppCompatActivity() {
    private var recyclerView: RecyclerView? = null
    private var toolbar: Toolbar? = null
    private var swipeRefreshRecyclerList: SwipeRefreshLayout? = null
    private var fab: FloatingActionButton? = null
    private var mAdapter: ToggleGridAdapter? = null
    private val modelList = ArrayList<AbstractModel>()
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_toggle_grid)
        findViews()
    ...
    private fun setAdapter() {
        ...
        mAdapter = ToggleGridAdapter(this@ToggleGridActivity, modelList)
        recyclerView!!.setHasFixedSize(true)
        val layoutManager = GridLayoutManager(this@ToggleGridActivity, 2)
        recyclerView!!.layoutManager = layoutManager
        recyclerView!!.adapter = mAdapter
        mAdapter!!.SetOnItemClickListener(object : ToggleGridAdapter.OnItemClickListener {
            override fun onItemClick(view: View?, position: Int, model: AbstractModel?) {

                //handle item click events here
                Toast.makeText(this@ToggleGridActivity, "Hey " + model!!.title, Toast.LENGTH_SHORT).show()
            }
        })
        mAdapter!!.SetOnCheckedListener(object : ToggleGridAdapter.OnCheckedListener {
            override fun onChecked(view: View?, isChecked: Boolean, position: Int, model: AbstractModel?) {
                Toast.makeText(this@ToggleGridActivity, (if (isChecked) "Checked " else "Unchecked ") + model!!.title, Toast.LENGTH_SHORT).show()
            }
        })
    }
}