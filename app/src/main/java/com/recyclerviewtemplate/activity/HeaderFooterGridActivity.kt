...
import com.recyclerviewtemplate.adapter.HeaderFooterGridAdapter
import java.util.*

class HeaderFooterGridActivity : AppCompatActivity() {
    private var recyclerView: RecyclerView? = null
    private var toolbar: Toolbar? = null
    private var swipeRefreshRecyclerList: SwipeRefreshLayout? = null
    private var fab: FloatingActionButton? = null
    private var mAdapter: HeaderFooterGridAdapter? = null
    ...
    private fun setAdapter() {
        ...
        mAdapter = HeaderFooterGridAdapter(this@HeaderFooterGridActivity, modelList, "Header", "Footer")
        recyclerView!!.setHasFixedSize(true)
        val layoutManager = GridLayoutManager(this@HeaderFooterGridActivity, 2)
        recyclerView!!.addItemDecoration(GridMarginDecoration(this@HeaderFooterGridActivity, 2, 2, 2, 2))
        recyclerView!!.layoutManager = layoutManager
        recyclerView!!.adapter = mAdapter
        mAdapter!!.SetOnItemClickListener(object : HeaderFooterGridAdapter.OnItemClickListener {
            override fun onItemClick(view: View?, position: Int, model: AbstractModel?) {

                //handle item click events here
                Toast.makeText(this@HeaderFooterGridActivity, "Hey " + model!!.title, Toast.LENGTH_SHORT).show()
            }
        })
        mAdapter!!.SetOnHeaderClickListener(object : HeaderFooterGridAdapter.OnHeaderClickListener {
            override fun onHeaderClick(view: View?, headerTitle: String?) {

                //handle item click events here
                Toast.makeText(this@HeaderFooterGridActivity, "Hey I am a header", Toast.LENGTH_SHORT).show()
            }
        })
        mAdapter!!.SetOnFooterClickListener(object : HeaderFooterGridAdapter.OnFooterClickListener {
            override fun onFooterClick(view: View?, footerTitle: String?) {

                //handle item click events here
                Toast.makeText(this@HeaderFooterGridActivity, "Hey I am a footer", Toast.LENGTH_SHORT).show()
            }
        })
    }
}