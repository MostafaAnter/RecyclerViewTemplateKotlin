...
import com.recyclerviewtemplate.adapter.BannerGridAdapter
import java.util.*

class BannerGridActivity : AppCompatActivity() {
    private var recyclerView: RecyclerView? = null
    private var toolbar: Toolbar? = null
    private var swipeRefreshRecyclerList: SwipeRefreshLayout? = null
    private var fab: FloatingActionButton? = null
    private var mAdapter: BannerGridAdapter? = null
    private val modelList = ArrayList<AbstractModel>()


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_banner_grid)
        findViews()
    ...
    private fun setAdapter() {
        ...
        mAdapter = BannerGridAdapter(this@BannerGridActivity, modelList)
        recyclerView!!.setHasFixedSize(true)
        val layoutManager = GridLayoutManager(this@BannerGridActivity, 2)
        recyclerView!!.addItemDecoration(GridMarginDecoration(this@BannerGridActivity, 2, 2, 2, 2))
        recyclerView!!.layoutManager = layoutManager
        recyclerView!!.adapter = mAdapter
        mAdapter!!.SetOnItemClickListener(object : BannerGridAdapter.OnItemClickListener {
            override fun onItemClick(view: View?, position: Int, model: AbstractModel?) {

                //handle item click events here
                Toast.makeText(this@BannerGridActivity, "Hey " + model!!.title, Toast.LENGTH_SHORT).show()
            }
        })
    }
}